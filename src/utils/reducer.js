    ...
    switch (action.type) {
        ...
        case 'ObjectPlus':
            ...
            if(dailyObject.f.length===0){
                dailyObject.f.push({id:0,object:action.payload,priority:0,status:0,startDate:(new Date().toLocaleDateString()),endDate:(new Date().toLocaleDateString()),objStartTime:e,objEndTime:e,Timelist:null,list:0})
            }else{
